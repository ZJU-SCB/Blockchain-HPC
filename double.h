...
#define __X_N 10001
double __X_matA[__X_N * __X_N], __X_matB[__X_N * __X_N],
    __X_tv_m[__X_N], __X_tv_m2[__X_N];
struct timespec __X_begin, __X_end;

void input(double *matA, double *matB)
{
    static int pid = -1;
    if (pid != -1)
    {
...
void output(double *result, int n)
{
    ...
    double *__X_tv = __X_tv_m, *__X_tv2 = __X_tv_m2;
    for (int i = 0; i < __X_N; ++i)
...
#pragma omp parallel for
    ...
    for (int i = 0; i < __X_N; ++i)
        ...
        {
            printf("Result is incorrect : shoul be %d but actual %d at %d,%d", __X_tv[i], result[test_on * __X_N + i], test_on, i);
            abort();
        }
